...
with a value outside of its value range. For example, if the boolean `b` is
provided a `nil` value instead of a boolean value, the target MUST reject this
operation by returning `INVALID_ARGUMENT`.
