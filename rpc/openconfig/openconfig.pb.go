...
// atomically applied.
//
// Deleting a non-existant, but schema valid, path is a noop.  Replacing or
// updating a non-existant, but schema valid, path will create the path.
//
// A delete operation on a path is the only way to remove an object. A replace
// or update operation with no value set or set to null or [] will only result
// in the object at the specified path being reset to schema, if provided, or
